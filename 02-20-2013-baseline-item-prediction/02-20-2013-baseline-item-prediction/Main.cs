...
namespace baselineitemprediction
{
	class MainClass
	{
		...
		static List<string> topNRatings(int N, Dictionary<string, double> avgRating) 
		{
			List<string> topNList = new List<string>();
			int count = 0;
			
			var sortedAvgRating = from pair in avgRating
		...
		static List<string> baselineAvgRating(int N, List<string> trainMovieIdList, List<int> trainRatingList) 
		{
			...
			for (int itr = 0; itr < trainRatingList.Count; itr++) {
				if (countUsers.ContainsKey(trainMovieIdList[itr])){
					countUsers[trainMovieIdList[itr]] = countUsers[trainMovieIdList[itr]] + 1;
				} else {
					countUsers.Add(trainMovieIdList[itr],1);
				}
				
				if (sumRatings.ContainsKey(trainMovieIdList[itr])){
					sumRatings[trainMovieIdList[itr]] = sumRatings[trainMovieIdList[itr]] + trainRatingList[itr];
				} else {
					sumRatings.Add(trainMovieIdList[itr],trainRatingList[itr]);
				}
			}
			...
			return topNRatings(N,avgRating);					
		}
		...
		{			
			...
			topNList = baselineAvgRating(N, trainMovieIdList,trainRatingList);
