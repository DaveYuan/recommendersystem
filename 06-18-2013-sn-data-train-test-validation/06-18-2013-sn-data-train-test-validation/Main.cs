...
/* To serialize Epinion-flixster-lastfm dataset with id mapping and create train test and validation split*/
namespace sndatatraintestvalidation
{
	class MainClass
	{
		...
		{			
			...
			for(int i = 0; i < valSize; i++) {
				int indx = r.Next(0, trainSize);	
				while (indxRemvTrain.Contains(indx)) {
					indx = r.Next(0, trainSize);
				}
		                                   ...
		                                   ref Dictionary<string, int> movieIdToIndx)
		{
			...
			int pseudoTrainSize = usersList.Count;
