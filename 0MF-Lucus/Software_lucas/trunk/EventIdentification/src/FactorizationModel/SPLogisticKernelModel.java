 ...
 *
 */
public class SPLogisticKernelModel extends GeneralModel {
		...
		for (int samp = 0; samp < 100; samp++) {
			...
			for (int i = 0; i < k; i++) {
				...
				for (int j = 0; j < f; j++) {
					...
					double truePart = - (derivativeOb1*featureVectors.get(ob1).get(j)*logOb2
							+logOb1*derivativeOb2*featureVectors.get(ob2).get(j));
					
					
					double falsePart = - (derivativeNegOb1*featureVectors.get(negob1).get(j)*logNegOb2
							+logNegOb1*derivativeNegOb2*featureVectors.get(negob2).get(j));
