...
public class ExperimentalMF extends MatrixFactorization {
	...
	public void initialize() {
		...
		for(int u = 0; u < userBias.length; u++){
			userBias[u] = ((double)userBias[u]+globalAverage*k)/(k+userRatings[u]);
			
		}
		for(int i = 0; i < itemBias.length; i++){
			itemBias[i] = ((double)itemBias[i]+globalAverage*k)/(k+itemRatings[i]);
