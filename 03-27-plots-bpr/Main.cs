...
namespace bprimproved
{
	class MainClass
	{
		...
		{														
			...
			for (epoch = 1; epoch <= this.bprEpochs; epoch++) {
				bprOpt = 0.0;				
				for (int n = 0; n < this.numTrainingExamples; n++) {	
					...
					for(int j = 0; j < this.numFeatures; j++) {	
						...
						userFeature[j,userIndx] += this.lrate * 
												( ( ( (Math.Exp(-xuij)) / (1 + Math.Exp(-xuij)) ) * dervXuij ) + this.K * uv );
					
						dervXuij = uv;
						itemFeature[j, itemIIndex] += this.lrate * 
												( ( ( (Math.Exp(-xuij)) / (1 + Math.Exp(-xuij)) ) * dervXuij ) + this.K * itemFeature[j, itemIIndex] );
					
						dervXuij = -uv;
						itemFeature[j, itemJIndex] += this.lrate * 
												( ( ( (Math.Exp(-xuij)) / (1 + Math.Exp(-xuij)) ) * dervXuij ) + this.K * itemFeature[j, itemJIndex] );															                                    					
					}
		 ...
		 */
		public void writeToFile(Dictionary<int, double> recallData,
		                        Dictionary<int, double> precisionData)
		{
			int itr = 0;
			string[] recallPerN = new string[this.numLines];		
			string[] precisionPerN = new string[this.numLines];	
			string[] precisionPerRecall = new string[this.numLines];	
		...
		{			
			...
			string[] testData = readAllLines("test.txt");
			...
			numTrainingExamples = readTrainSet("train.txt", 
			             						ref userIdIndxInfo, 
			             						ref itemIdIndxInfo,
			...
			mainclass.writeToFile(recallData,
			                      precisionData);
		}		
	}
}